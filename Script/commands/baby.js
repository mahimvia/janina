...
module.exports.run = async function ({ api, event, args }) {
  ...
  // If no input, reply with a random cute reply
  if (!input) {
  ...
  // Teach mode: .baby teach প্রশ্ন = উত্তর
  if (input.toLowerCase().startsWith("teach ")) {
    const content = input.slice(6).trim();
    const [ask, ans] = content.split("=").map(s => s.trim());
    if (!ask || !ans) {
      return api.sendMessage("❌ সঠিক ফরম্যাট:\n.baby teach প্রশ্ন = উত্তর", threadID, messageID);
    }
    try {
...
// Unprefixed keywords listener (auto-reply to certain keywords, exact match)
module.exports.handleEvent = async function({ api, event }) {
  const { body, threadID, messageID, senderID } = event;
  if (!body) return;

  // Normalize text to lower case and trim spaces
  const text = body.trim().toLowerCase();

  // If message matches a trigger word exactly, reply via SimSimi API
  if (triggerWords.includes(text)) {
    try {
      const res = await axios.get(`${baseApiUrl}/sim?text=${encodeURIComponent(text)}`);
      const reply = res.data.reply || "🤔 আমি ঠিক বুঝতে পারিনি, আবার বলো তো!";
      return api.sendMessage(reply, threadID, (err, info) => {
        if (!err) global.client.handleReply.push({
          name: module.exports.config.name,
      ...
      console.error("Unprefix Chat API Error:", err.message);
      return api.sendMessage("⚠️ উত্তর আনতে সমস্যা হয়েছে!", threadID, messageID);
    }
  }
};

// For replying to follow-up messages (handleReply)
